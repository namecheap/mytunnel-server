...
const DEFAULT_GRACE_TIMEOUT = 50000;
...
class Client extends EventEmitter {
    ...
    handleRequest(req, res) {
        ...
        clientReq.once('error', (err) => {
            this.debug('client request error %o', err);
            // if headers not sent - respond with gateway unavailable
            if (!res.headersSent) {
                res.statusCode = 502;
                res.end('Tunnel: Bad Gateway');
            }
        });
    ...
    handleUpgrade(req, socket) {
        ...
        socket.once('error', (err) => {
            ...
            this.debug('socket upgrade error %o', err);
        });
        ...
        this.agent.createConnection({}, (err, conn) => {
            ...
            if (err) {
                this.debug('upgrade create connection error %o', err);
                socket.end();
                return;
            }
            ...
            if (!socket.readable || !socket.writable) {
                this.debug('upgrade source socket not available');
                conn.destroy();
                socket.end();
                return;
            }
