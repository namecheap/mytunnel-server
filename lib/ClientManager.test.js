...
describe('ClientManager', () => {
  ...
  it('should remove correct client once it goes offline', async () => {
    ...
    // Get the actual client objects to listen for online event
    const fooClient = manager.getClient('foo');

    // Set up online listener BEFORE connecting
    const onlinePromise = new Promise((resolve) => {
      fooClient.agent.once('online', resolve);
    });
    ...
    // Wait for the client to go online (this clears the grace timeout)
    await onlinePromise;

    // Now tick the clock - foo should stay (has connection), bar should be removed (no connection)
    await clock.tickAsync(50_001);

    // foo should still be ok (has active connection)
    assert(manager.hasClient('foo'), 'Client foo should still exist');
    // clientBar should be removed - nothing connected to it
    assert(!manager.hasClient('bar'), 'Client bar should be removed');
